...
Config = {
    ...
    "Logs": 1202416795227521024, # log channel
    "Crash" : { # Config for crash
        ...
        "ChannelID" : "1200843423675076741"  # Id of the channel crash games will be in
    },
    "Coinflip" : { # Config for coinflip
        "1v1" : "1202420155452497940",  # Channel That Coinflips Be In
        "House": 3.5 # House Edge (%)
    },
    "Rains" : { # Config for rains
        "Channel" : "1202417956835954698" # Set to the id the channel rains will be in
    },
    "Upgrader": { # Config for upgrader
        "House": 0.95 # house edge (winnings*house)
    },
    "Rakeback" : 1, # Rakeback %
    "Username": "WhyYouSpying", # The Username Of The Account Running The Bot
    "DiscordBotToken": "MTEyMTU0MzY2OTI4ODI4MDA2NA.GbHp95.Bg2kOR9YJ8PRzCphYT0_8_LDQQ-NkZYwNQrFl4" # The token of the discord bot
}
...
async def withdraw(interaction: discord.Interaction, amount: str, uname: str) :
    ...
    if is_registered(str(interaction.user.id)) :
        if get_gems(str(interaction.user.id)) >= amount :
            ...
            embed = discord.Embed(title=":x: Error",
                                  description="You Are Too Poor For This Withdraw",
                                  color=0xff0000)
            embed.set_author(name="Pet Sim 99 Gamble Bot",
                             icon_url="https://creazilla-store.fra1.digitaloceanspaces.com/cliparts/7803751/diamond-gem-clipart-sm.png")
            embed.set_footer(text="/withdraw")
...
@bot.tree.command(name="create-rain", description="Creates a Rain Using Your Gems")
...
@bot.tree.command(name="flip", description="1v1 Coinflip")
